...
本地开发服务管理面板。把本地项目集中到一个页面里，统一查看在线状态、访问地址、启动、停止、重启项目，适合同时维护多个前端、后端、脚本服务。
...
随着Vibe Coding越来越上头，以下这些问题开始频繁困扰我：记不住都开发了哪些项目、记不住项目端口号、记不住后台运行着哪些服务、记不住项目启动命令。

所以我做了这个LocalServiceHub，目标很简单：把这些本地服务变成一组可见、可操作的服务卡片。

但是，我不想去记任何命令行、也不想去记任何使用方法，我不想为了解决一个问题，再引入一个新问题。所以这个工具，你不需要记任何东西，你只要启动它，然后复制提示词到你的coding Agent里就可以了。

## 亮点

- **AI 注册**：项目内置注册提示词，复制到你的Coding Agent里，让 AI 分析项目结构并调用LocalServiceHub接口完成注册。
...
- 🌟推荐：点击「注册提示词」，复制提示词到目标项目的 AI 对话中，让 AI 分析项目并调用注册接口。
